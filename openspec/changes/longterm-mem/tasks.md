...
- [x] 10a.1 RED `engine/runtime/longtermmem_test.go::TestLongtermMemAdapter_StatusMatrix` (table-driven) — supported (binary executable ∧ install-state record ∧ entry present with matching fingerprint); partial ×4 named reasons (record without entry, entry without record/unmanaged, fingerprint drift, missing binary); unsupported (config root unresolvable).
- [x] 10a.2 RED (same file) `TestLongtermMemAdapter_InstallRecordsRegistrationAndReportsStatus` — GIVEN the binary already built/copied, WHEN `Install()` runs, THEN `registration.json` is written and a per-runtime status is reported for claude/opencode/codex. Scenario: "Install records registration and reports per-runtime status" (runtime-lifecycle ADDED requirement, R-014).
- [x] 10a.3 RED (same file) `TestLongtermMemAdapter_StatusAndUninstallRequireNoBuild` — `Status()`/`Uninstall()` read/remove the registration directly, no build step. Scenario: "Status and uninstall report without requiring a build".
- [x] 10a.4 RED (same file) `TestLongtermMemAdapter_UpdateAndRollbackRefused` — `Update()`/`Rollback()` return a refusal rather than attempting an action. Scenario: "No update or rollback surface is offered".
- [x] 10a.5 GREEN `engine/runtime/longtermmem.go` — `LongtermMemAdapter` implementing `runtime.Adapter` (D4): stdlib-only, read-only config inspection (JSON decode for claude/opencode, TOML header/`command =` line scan for codex); `registration.json` record read/write; the status vocabulary above; `Update()`/`Rollback()` return `CapabilityUnsupported` with an explicit reason. Landed as two files (`longtermmem.go` core + `longtermmem_config.go` read-only inspection) — see apply-progress.md for the seam rationale.
- [x] 10a.6 RED `engine/cmd/main_test.go::TestComponentFlag_LongtermMemRefusesUpdateRollback` — `--component longterm-mem update` / `... rollback` parsed at the CLI layer; assert exit 1 before any adapter call (D4 parse-time refusal).
- [x] 10a.7 RED (same file) `TestComponentFlag_DefaultIsRuntimeParity` — no `--component` flag behaves exactly as today (regression guard).
- [x] 10a.8 GREEN `engine/cmd/main.go` — `--component {runtime-parity|longterm-mem}` and `--state-dir` flags; component dispatch selecting `LongtermMemAdapter` vs. the existing parity adapters; usage text update.
- [x] 10a.9 REFACTOR — factor shared claude/opencode/codex config-inspection primitives between `LongtermMemAdapter` and the existing parity adapters where the schema overlaps. Root-resolution funcs and a new `readJSONObjectOrNil` helper (also adopted by `ClaudeAdapter.status()`) are shared; codex's TOML-scan and OpenCode's strongly-typed plugin config are deliberately NOT forced together — see apply-progress.md.
- [x] 10a.10 Slice verification — `cd engine && go test ./...`.
...
- [x] 10b.1 RED `engine/installer/route_test.go::TestInstall_BuildsCopiesThenReportsPerRuntimeStatus` (integration, `-short`-skippable, sandbox HOME/OVERLAY_DIR/STATE_DIR) — `longterm-mem install --target all`; assert a binary exists at `$STATE_DIR/bin/longterm-mem` afterward and a per-runtime status is reported for claude/opencode/codex. Scenario: "Install builds, copies, then reports per-runtime status" (install spec R-014).
- [x] 10b.2 RED (same file) `TestStatusUninstall_SkipBuildStep` — `longterm-mem status` / `longterm-mem uninstall`; assert no build invocation occurs. Scenario: "Status and uninstall skip the build step" (R-014).
- [x] 10b.3 RED (same file) `TestInstall_BinaryPersistsAfterProcessExits` — fresh install; installing process exits; assert the binary still exists at the documented fixed path and is invocable. Scenario: "Binary persists after the installing process exits" (R-015).
- [x] 10b.4 RED (same file) `TestInstall_BinaryPathStableAcrossInspections` — no install/uninstall in progress; inspect the path at two points; assert unchanged. Scenario: "Binary path is stable absent install/uninstall activity" (R-015).
- [x] 10b.5 GREEN `bin/labdrian-overlay` — `cmd_longterm_mem()` dispatcher: `install|status|uninstall [--target claude|opencode|codex|all] [--purge]`; `install` = `go build ./longterm-mem/cmd/longterm-mem` → copy to `LONGTERM_MEM_BINARY="$STATE_DIR/bin/longterm-mem"` → `longterm-mem vaults seed` → per target (`longterm-mem register --target t` then `engine runtime install --component longterm-mem --target t`); `status`/`uninstall` skip the build step; binary removed only when `--target all` leaves zero install-state targets, or `--purge`. See apply-progress.md Slice 10b for the `LONGTERM_MEM_SRC`-cd and own-tracked-targets deviations from the literal task text (both required, both documented, neither changes the observable contract).
- [x] 10b.6 GREEN `bin/labdrian-overlay` — usage text + top-level dispatcher case for `longterm-mem`; `LONGTERM_MEM_BINARY="$STATE_DIR/bin/longterm-mem"` constant.
- [x] 10b.7 GREEN `bin/labdrian-overlay` — `cmd_apply()` hook: after the existing copy loop, any manifest row whose `route_resolve` result is `mcp` sets a flag; `cmd_apply` calls `cmd_longterm_mem install --target "$TARGET"` once, after the copy loop (D13), reusing `route_reject_unrouted_longterm_mem()` (9.11) for the same-pass rejection (inherited automatically: `route_resolve` already calls that guard for every `longterm-mem/*` row `cmd_apply` processes). Proved with a dedicated RED/GREEN integration test (`TestApply_InvokesLongtermMemInstallOnceForMcpRow`, not separately enumerated in this task list) exercising the REAL success path end-to-end, per the apply brief's explicit 10a-postmortem callout.
- [x] 10b.8 REFACTOR `longterm-mem/README.md` — refresh (CHK-05): per-project wording, fixed install path, full CLI/MCP surface now that `install`/`status`/`uninstall` exist end-to-end.
- [x] 10b.9 Slice verification (10a+10b) — `cd engine && go test ./... && cd tui && go test ./... && bash -n bin/labdrian-overlay && bash -n bin/overlay`; `cd longterm-mem && go test ./...`.
...
- [x] 11b.1 RED `longterm-mem/internal/register/claude_test.go::TestClaude_UnrelatedEntriesPreserved` — golden fixture `~/.claude.json` with unrelated `mcpServers` entries; assert install adds only a new ownership-tagged `longterm-mem` entry, all pre-existing entries byte-identical. Scenario: "Unrelated entries are preserved" (R-016).
- [x] 11b.2 RED (same file) `TestClaude_ReinstallIsIdempotent` — existing tagged entry; re-install; assert in-place replace, not a duplicate. Scenario: "Reinstall is idempotent" (R-016).
- [x] 11b.3 RED (same file) `TestClaude_UntaggedSameNamedEntryRefused` — an untagged `longterm-mem`-named entry install-state does not own; assert refusal + reported conflict (exit 6), nothing written. Scenario: "Untagged same-named entry is refused, not overwritten" (R-016).
- [x] 11b.4 GREEN `longterm-mem/internal/register/claude.go` — `RegisterClaude(configRoot, binary string) error` writing `"longterm-mem": {"type":"stdio","command":"<bin>","args":["mcp"]}` under `mcpServers` via `jsonsplice.go` + `installstate.go`; golden fixtures `testdata/claude/*`. Landed as `RegisterClaude(configRoot, stateDir, binary string) error` — see apply-progress.md Slice 11b for the added-parameter rationale.
- [x] 11b.5 RED `longterm-mem/internal/register/opencode_test.go::TestOpencode_UnrelatedEntriesPreserved` / `TestOpencode_ReinstallIsIdempotent` / `TestOpencode_UntaggedSameNamedEntryRefused` — same three scenarios against opencode's `opencode.json` `mcp` map. Scenarios: "Unrelated entries are preserved", "Reinstall is idempotent", "Untagged same-named entry is refused, not overwritten" (R-017).
- [x] 11b.6 GREEN `longterm-mem/internal/register/opencode.go` — `RegisterOpencode(configRoot, binary string) error` writing `"longterm-mem": {"type":"local","command":["<bin>","mcp"],"enabled":true}`; golden fixtures `testdata/opencode/*`. Landed as `RegisterOpencode(configRoot, stateDir, binary string) error` — same rationale as 11b.4.
- [x] 11b.7 GREEN `longterm-mem/cmd/longterm-mem/cmd_register.go` — `register --target claude|opencode|codex|all [--config-root DIR] [--state-dir DIR] [--binary PATH]` wiring (codex target wired in 12a.6). Landed as two files — `register_paths.go` (pure target/default-path resolution) and `cmd_register.go` (flag parsing, dispatch, exit codes) — split at 419 changed lines rather than requesting a size exception; see apply-progress.md Slice 11b.
- [x] 11b.8 REFACTOR — extract the golden-fixture harness (`testdata/<runtime>/before.json`, `after-install.json`, `after-reinstall.json`, `after-uninstall.json`) for reuse by codex in slice 12a. Built up front as `golden_writer_test.go` (driving claude.go from 11b.1) rather than extracted after the fact from two near-duplicate test files — see apply-progress.md Slice 11b.
- [x] 11b.9 Slice verification — `cd longterm-mem && go test ./...`.
...
- [x] 12a.1 RED `longterm-mem/internal/register/tomlsplice_test.go::TestTOMLSplice_LocatesTableSpan` — fixture `config.toml` with an existing unrelated `[mcp_servers.other]` section and unrelated top-level keys; assert the header regex `^\s*\[mcp_servers\.("longterm-mem"|longterm-mem)\]` locates the span to the next `^\s*\[` or EOF, replaces only that span, all else unchanged, file stays valid via `go-toml/v2` parse-only validation. See apply-progress.md Slice 12a for the blank-line-separator refinement this test's RED failure forced.
- [x] 12a.2 GREEN `longterm-mem/internal/register/tomlsplice.go` — TOML table byte-splice editor (D9), `.bak`, tmp+rename, post-write `pelletier/go-toml/v2 Unmarshal` + `mcp_servers.longterm-mem.command == binary` validation before rename. Landed as two files — `tomlsplice.go` (pure locate/apply) + `tomlwrite.go` (atomic writer + validation) — the same purity-seam split 11a.3 used for JSON; see apply-progress.md Slice 12a.
- [x] 12a.3 RED `longterm-mem/internal/register/codex_test.go::TestCodex_UnrelatedSectionsAndOrderingPreserved` — golden fixture; assert only a new `longterm-mem` section is added, all other sections/key ordering unchanged, file stays valid. Scenario: "Unrelated sections and ordering are preserved" (R-018).
- [x] 12a.4 RED (same file) `TestCodex_ReinstallIsIdempotent` — existing tagged section; re-install; assert in-place replace. Scenario: "Reinstall is idempotent" (R-018). A third scenario, `TestCodex_UntaggedSameNamedEntryRefused`, was added alongside these two — see apply-progress.md Slice 12a.
- [x] 12a.5 GREEN `longterm-mem/internal/register/codex.go` — `RegisterCodex(configRoot, binary string) error` writing `[mcp_servers.longterm-mem]\ncommand = "<bin>"\nargs = ["mcp"]` via `tomlsplice.go` + `installstate.go`; golden fixtures `testdata/codex/*`. Landed as `RegisterCodex(configRoot, stateDir, binary string) error`, same added-parameter rationale as 11b.4/11b.6, driven through the new shared `tomlInstall` (writer.go), codex's counterpart to `jsonInstall`.
- [x] 12a.6 GREEN — wire `codex` into `cmd_register.go` (11b.7) target dispatch and `registerExpandTarget`'s `"all"` expansion (`register_paths.go`); updated `TestRegisterExpandTarget`'s `"all"` row and `TestCmdRegister_AllExpandsToClaudeAndOpencode` (renamed `...AndCodex`) accordingly. Exit-precedence (hard failure outranks conflict) reverified unchanged.
- [x] 12a.7 REFACTOR — confirmed `internal/register` package doc names all three runtimes' D9 semantics table in one place. Extracted the package doc from `jsonsplice.go` into a new `doc.go` (it was never JSON-specific) and added the three-boolean-to-Action table plus an explicit note that engine/runtime's `doctor`/`status` reporting re-derives the same states read-only from a separate module, per D4.
- [x] 12a.8 Slice verification — `cd longterm-mem && go test ./...`; `cd engine && go test ./...`.
...
- [x] 12b.1 RED `longterm-mem/internal/register/unregister_test.go::TestUnregister_SelectiveRemovalAcrossAllThreeRuntimes` — claude/opencode/codex configs each carry a longterm-mem entry plus unrelated entries; uninstall one runtime; assert only that runtime's entry is removed, all other entries in all three files untouched. Scenario: "Selective removal across all three runtimes" (R-019). Landed alongside `Remove`/`TOMLRemove` position-dependent RED tests in `jsonsplice_test.go`/`tomlsplice_test.go` (first/middle/last member/table); see apply-progress.md Slice 12b.
- [x] 12b.2 RED (same file) `TestUnregister_UntaggedEntryPreservedAndReported` — untagged `longterm-mem`-named entry install-state does not own; uninstall; assert left in place, reported `unmanaged`, not removed. Scenario: "Untagged entry is preserved and reported, not removed" (R-019). Run across all three runtimes via the golden harness's new `testUninstallUntaggedEntryPreservedAndReported` method, reusing each runtime's existing `untagged.<ext>` fixture.
- [x] 12b.3 RED (same file) `TestUnregister_PartialUninstallKeepsSharedBinary` — uninstall from only one of the three runtimes; assert the shared binary is not removed. Scenario: "Partial uninstall does not remove the shared binary" (R-019). The Go-level proof: with all three targets recorded in one `install-state.json`, unregistering one target leaves the other two targets' records AND config entries fully intact — see apply-progress.md Slice 12b for why this, not the (bash-owned) binary file itself, is this package's half of the guarantee.
- [x] 12b.4 GREEN `longterm-mem/internal/register/unregister.go` — `Unregister(target, configRoot, stateDir string) (UnregisterOutcome, error)`: record present → remove span (and any dangling comma/blank line) via the same splice editors; no record → leave + report `unmanaged`; updates `install-state.json`. Landed as a single dispatcher function (not three per-runtime wrappers) since unregister has no per-runtime entry SHAPE to write, only somewhere to look; `jsonUninstall`/`tomlUninstall` (writer.go) do the actual Decide-driven work.
- [x] 12b.5 GREEN `longterm-mem/cmd/longterm-mem/cmd_unregister.go` — `unregister --target claude|opencode|codex|all` wiring.
- [x] 12b.6 GREEN `bin/labdrian-overlay` — `cmd_longterm_mem uninstall`: `unregister` then `engine runtime uninstall --component longterm-mem`; `--purge` flag glue; binary removed only when `--target all` leaves zero install-state targets or `--purge`.
- [x] 12b.7 REFACTOR — final pass so `claude.go`/`opencode.go`/`codex.go` share one `Decide` call site (11a.6) for both install and uninstall paths. `jsonUninstall`/`tomlUninstall` (writer.go) call the exact same `Decide` function `jsonInstall`/`tomlInstall` do, reinterpreting its four outcomes for the uninstall direction (see `UnregisterOutcome`'s doc comments, unregister.go) rather than adding a parallel decision table.
- [x] 12b.8 Slice verification (12a+12b, full change) — `cd longterm-mem && go test ./...`; broad: `cd longterm-mem && go test ./... && cd engine && go test ./... && cd tui && go test ./... && bash -n bin/labdrian-overlay && bash -n bin/overlay`.
- Command: `cd longterm-mem && go test ./... -run TestUnregister`

---

## Slice 13 — verify remediation (CRITICAL-1, CRITICAL-2, dead-code closure) — PR13-{1,2,3,4} (base: PR12b)

`sdd-verify`'s report (`verify-report.md`) found 80/82 scenarios compliant
with two CRITICAL findings against R-019 and R-035, both proved by
execution, plus three unreachable production functions. This slice fixes
exactly those findings under strict TDD; no other production surface is
touched.

- [x] 13.1 RED `engine/skills/ondisk_test.go::TestDeployableManifestPaths_RejectsUnroutedLongtermMemRow` / `TestDeployableManifestPaths_RejectsUnrecognizedRouteLongtermMemRow` — a `longterm-mem/**` manifest row with a missing third column, and one with an unrecognized route value; assert `DeployableManifestPaths` returns a non-nil error naming the row AND never resolves it to a skills-destination path. Real failure captured before the fix: `DeployableManifestPaths: expected an error ..., got nil (paths=map[longterm-mem/internal/foo.go:{} ...])` for both. Scenario: "Unrouted longterm-mem row is rejected by both parsers" (overlay-agent-route R-012, traces longterm-mem R-035). CRITICAL-2.
- [x] 13.2 GREEN `engine/skills/ondisk.go` — `DeployableManifestPaths` gains a `validLongtermMemRoutes` domain check scoped to rows under `longterm-mem/**`: a missing or unrecognized third column returns an explicit `fmt.Errorf` naming the row, mirroring `route_reject_unrouted_longterm_mem` in `bin/labdrian-overlay` (the bash half already correct). `nonSkillRoutes` is untouched — `skill` stays a valid default for every other row. `engine/go.mod` still declares no `require` block (`TestZeroFetchImportAllowlist` re-verified green).
- [x] 13.3 Added `TestDeployableManifestPaths_LongtermMemRouteGuardAcceptsEveryValidRoute` (triangulation) — every value in `{skill, agent, opencode-agent, mcp}` on a `longterm-mem/**` row parses without error, proving the guard rejects only a missing/unrecognized route, not the skill route specifically.
- Command: `cd engine && go test ./skills/... -run TestDeployableManifestPaths`
- [x] 13.4 RED `engine/installer/route_test.go::TestInstall_UninstallRoundTripRemovesTheMcpEntry` — real `longterm-mem` binary (real `go build`, not the two-line dummy stub `TestStatusUninstall_SkipBuildStep` uses), real `bin/labdrian-overlay install --target claude` then `uninstall --target claude` against a seeded `~/.claude.json`; assert the ownership-tagged `longterm-mem` MCP entry is present after install and ACTUALLY ABSENT after uninstall, with the unrelated pre-existing entry untouched throughout. Real failure captured before the fix: the entry was still present after uninstall (`CRITICAL-1 regression: the longterm-mem MCP entry is still present after uninstall ...`). Scenario: "Selective removal across all three runtimes" (longterm-mem-mcp-registration R-019).
- [x] 13.5 RED (same file) `TestUninstall_HardFailureKeepsTrackingAndSharedBinary` — corrupts `install-state.json` after a real install so the next `unregister` call returns a genuine hard failure (exit 1, not the recoverable exit 6/"unmanaged"); assert the shared binary is NOT rebuilt/removed and the bash-level tracking still lists the target as installed. Real failure captured before the fix (old code): unregister reported "unmanaged" (wrong `--state-dir` meant it never even reached the corrupted file), the exit code was swallowed, tracking was cleared, and — reproducing the report's documented blast radius — the shared binary WAS removed: `==> longterm-mem binary removed: .../bin/longterm-mem`.
- [x] 13.6 GREEN `bin/labdrian-overlay` — `cmd_longterm_mem`: (a) removed the wrong `--state-dir "$STATE_DIR"` argument from the `unregister` call so it resolves the same `install-state.json` location `register` (install) does — the module's own default, `~/.labdrian-overlay/longterm-mem` (D9); (b) `unregister`'s real exit status is now inspected instead of swallowed by `|| warn ... continuing`: exit 0 (removed) and exit 6 (unmanaged — an entry longterm-mem does not own) clear that target's bash-level tracking; any other exit status keeps the target tracked, skips the engine-registration/binary-removal branch (unless `--purge` forces it), and makes the whole `uninstall` invocation return 1; (c) removed the `longterm-mem vaults seed` call and its misleading "later slice" warning — that subcommand was never planned (`vaults` has no `cmdVaults` case in `cmd/longterm-mem/main.go`) and R-022 is satisfied by lazy seeding in `vaultreg.Resolve`, per the report's WARNING-1. CRITICAL-1.
- Command: `cd engine && go test ./installer/... -run TestInstall_UninstallRoundTripRemovesTheMcpEntry|TestUninstall_HardFailureKeepsTrackingAndSharedBinary`
- [x] 13.7 RED `longterm-mem/cmd/longterm-mem/main_test.go::TestCmdStatus_ReportsDegradedEngramConnection` — forces `engram.Open`'s real `immutable=1` fallback (unwritable DB directory, no `-wal`/`-shm` on disk, mirroring `internal/engram/store_test.go`'s own `TestOpen_FallsBackToImmutableWhenPrimaryReadOnlyOpenFails` fixture) and asserts `longterm-mem status`'s `engram:` output line reports both `reachable=true` AND the degraded detail. Real failure captured before the fix: `engram line does not surface the degraded fallback ...: "  engram: reachable=true"`.
- [x] 13.8 GREEN `longterm-mem/cmd/longterm-mem/cmd_status.go` — the `EngramReachable` seam now calls `store.Degraded()` and, when true, returns a detail string naming the stale fallback and its cause, instead of unconditionally returning `true, ""`. This is the only one of the three unreachable functions the report identified as genuinely missing behavior (`WARNING-2`) rather than truly dead, so it is wired in rather than deleted.
- [x] 13.9 DELETE `longterm-mem/internal/engram/store.go::Store.Path()` — no production caller and no missing-behavior case (unlike `Degraded`); deleted along with the two assertions that used it in `store_test.go` (`TestOpen_DefaultIsReadOnly`, `TestOpen_OverridePathStaysReadOnly` — both test functions themselves stay, since they pin R-002's read-only scenarios independently of `Path()`). The now-unused `Store.path` field was removed with it.
- [x] 13.10 DELETE `longterm-mem/internal/register/decide.go::Action.String()` — no caller anywhere, not even a test; its own doc comment claimed a use (`ErrConflict` reporting) that the code never actually took. Deleted with no test to remove (none existed). `UnregisterOutcome.String()` (a live, reachable method `cmd_unregister.go` calls through `%s`) had its doc comment corrected to no longer reference the now-deleted `Action.String()`.
- Command: `cd longterm-mem && go test ./... -run TestCmdStatus_ReportsDegradedEngramConnection` ; `go run golang.org/x/tools/cmd/deadcode@latest ./cmd/longterm-mem` (zero findings, was three)
- [x] 13.12 GREEN `bin/labdrian-overlay` — the convergence rule the resilience lens required (`R4-uninstall-never-converges`, 13-2's bounded correction). The exit-status branching of 13.6 made `uninstall` non-convergent for states no operator action resolves: a missing or non-executable binary yields 127 and a version-skewed one yields 2, so every retry reproduced the state, kept the target tracked, and left the shared binary and engine record stranded — with `--purge`, the one action that orphans the entries, as the only escape. Added an `unregister_usable` pre-check (absent or non-executable binary: say what could not be removed, clear tracking, converge) and an explicit exit-2 branch with the same disposition. Exit 1 keeps its blocking behaviour, because unreadable configs, corrupted install-state and permissions are operator-fixable and a re-run after the fix can succeed.
- [x] 13.13 RED `engine/installer/route_test.go::TestUninstall_MissingBinaryStillConverges` — install for real, delete the binary, then uninstall; assert the run converges (exit 0), names the entries left behind, and does not keep the target tracked. Real failure captured before 13.12: `uninstall did not converge with the binary missing: exit status 1`, with `WARN: longterm-mem unregister --target claude failed (exit 127); keeping it tracked as installed`.
- [x] 13.14 RED (same file) `TestUninstall_VersionSkewStillConverges` — the other half of the same rule, which 13.12 had shipped with only manual proof (`verify-report` run 2, WARNING-3). Replaces the installed binary with one that runs and exits 2, the way an older build would; asserts the run converges, names the version skew and the entry left behind, and clears tracking. Proved load-bearing by routing exit 2 into the blocking branch: `uninstall did not converge against a version-skewed binary: exit status 1`. (Unlinking the binary before rewriting it is required — the just-executed file is still mapped, so writing in place fails with ETXTBSY.)
- [x] 13.11 Slice verification (full remediation) — `cd longterm-mem && gofmt -l . && go vet ./... && go test ./... -cover -count=1`; `cd engine && go test ./...`; `cd tui && go test ./...`; `bash -n bin/labdrian-overlay && bash -n bin/overlay`; `shellcheck bin/labdrian-overlay` (byte-identical pre-existing findings only, confirmed against the pre-change baseline — no new findings); `TestOSExecImportAllowlist` and `TestZeroFetchImportAllowlist` re-run individually, both PASS.
