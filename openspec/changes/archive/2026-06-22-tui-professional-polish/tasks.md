...
- [x] 4.1 **[RED R-008]** Write unit test `TestBuildArgSets` in `tui/main_test.go` (or `tui/run_test.go`): assert `buildArgSets(Action{TargetAgnostic: true, Command: "status-hooks"}, nil, false)` returns `[["status-hooks"]]` with no `--target`; assert non-TargetAgnostic action with one target returns `[["apply", "--target", "agent-x"]]`. Run → fail (function does not exist yet).
- [x] 4.2 **[GREEN R-008]** Extract `buildArgSets(action Action, selected []Target, allSelected bool) [][]string` pure function from `runBackend` in `tui/run.go`; add `TargetAgnostic bool` and `ConfirmMessage string` fields to `Action` struct in `tui/run.go` with gofmt realignment; add `TargetAgnostic` branch as first case in the switch. Run `TestBuildArgSets` → green.
- [x] 4.3 **[RED R-009]** Write unit test `TestHooksActionsRegistered` in `tui/main_test.go`: inspect `Actions()` return; assert "status-hooks", "install-hooks", "uninstall-hooks" all present with `TargetAgnostic: true`; assert `status-hooks` has `Mutating: false`; assert install/uninstall have `Mutating: true`. Run → fail.
- [x] 4.4 **[GREEN R-009]** Append three hooks `Action` entries to `Actions()` in `tui/run.go` with Spanish display names, correct `Mutating`/`TargetAgnostic` values, and `ConfirmMessage` copy for install/uninstall (Spanish, mentioning `settings.json` and `.bak`). Run `TestHooksActionsRegistered` → green.
- [x] 4.5 **[VERIFY R-008 assumption]** Manually test `bin/overlay status-hooks --target some-target` against the live engine: confirm it errors or rejects `--target`. Document the observed behavior as a comment in `tui/run.go` above the `TargetAgnostic` branch. (This is a verification step, not a code change; if the engine accepts `--target` silently, note it as a risk in the PR description.) — VERIFIED: cmd_status_hooks, cmd_install_hooks, cmd_uninstall_hooks receive "$@" but parse NO arguments; extra flags are silently ignored. Behavior documented in buildArgSets comment in run.go.

### Phase 5 — Routing + Rendering (model.go + view.go)

- [x] 5.1 **[RED R-010]** Write unit test `TestConfirmMessageSelection` in `tui/main_test.go`: model with `pendingAction.ConfirmMessage == ""` on screenConfirm → rendered output contains the generic phrase "Esta acción modifica los destinos"; model with install-hooks action → rendered output contains "settings.json" and ".bak". Run → fail.
- [x] 5.2 **[GREEN R-010]** Update `viewConfirm` in `tui/view.go` to branch on `m.pendingAction.ConfirmMessage != ""` → use custom copy; else → existing generic copy. For `TargetAgnostic` actions, omit the "en: <targets>" line. Run `TestConfirmMessageSelection` → green.
- [x] 5.3 **[RED R-009]** Write teatest `TestHooksSeparatorVisible` in `tui/main_test.go`: drive model to screenHome, render, assert output contains "─── Hooks ───". Run → fail.
- [x] 5.4 **[GREEN R-009]** Add separator rendering to `viewActions` loop in `tui/view.go`: emit `dimStyle.Render("  ─── Hooks ───")` immediately before the first `TargetAgnostic` action, using the boundary-detection logic from the design. Run `TestHooksSeparatorVisible` → green.
- [x] 5.5 **[RED R-009]** Write teatest `TestStatusHooksSkipsConfirm` in `tui/main_test.go`: navigate to "status-hooks" action, press enter, assert model goes to `screenRunning` directly (not `screenConfirm`). Also write `TestInstallHooksRequiresConfirm`: navigate to "install-hooks", press enter, assert `screenConfirm`. Run → fail (or verify existing `Mutating` routing already handles this — confirm either way).
- [x] 5.6 **[GREEN R-009]** Confirm routing: `status-hooks` has `Mutating: false` → existing `updateActions` path already skips confirm (model.go:173). `install-hooks` / `uninstall-hooks` have `Mutating: true` → existing path routes to confirm. If any gap exists, patch `tui/model.go`. Run `TestStatusHooksSkipsConfirm` and `TestInstallHooksRequiresConfirm` → green.

### Phase 6 — PR-2 Verification Gate

- [x] 6.1 **[R-011]** Run `cd tui && go test ./...`: confirm all pre-existing + PR-1 + PR-2 tests green. Zero failures. (26/26 pass, coverage 76.9%)
- [x] 6.2 **[R-011]** Run `go vet ./...` and `go build ./...` from `tui/`: no errors.
- [x] 6.3 **[R-012]** Review all string literals added in PR-2 diff: hooks display names and confirm copy are in Spanish; `Cmd` fields ("status-hooks" etc.) are CLI names and may remain English.
- [x] 6.4 **[Backwards compat]** Assert in test or review that `apply` and `capture` `Action` entries have empty `ConfirmMessage` and `TargetAgnostic: false` — zero-value defaults preserve prior behavior. (TestBackwardsCompatZeroValues green)
