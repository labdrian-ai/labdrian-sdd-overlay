...
| `skills/sdd-time-estimation/SKILL.md` | Modify | Line 28: "(tiering-go-ahead to archive, …)" → merge; CALIBRATION list item gains the R-019 absent-numerators fallback sentence ("… regardless of calibration `n`"); R-009 pins unaffected |
| `engine/skills/testdata/corrected-actuals-sync-check-repo-behind-origin.json` | Modify | Append provenance sentence (D7) |
| `engine/skills/actuals_instrumentation_d2_anchor_test.go` | Create | Proves D2 revision 3's four-state anchor resolution (`absent`/`self-asserted`/`verified`/`rejected`) against real repository history, and reads shipped `SKILL.md` prose at runtime to bind the proof to the shipped rule — see the Revert note below: this is why deleting it, not just the other 5 edits, is mandatory |
| `engine/skills/testdata/corrected-actuals-skills-validate-ondisk-gate.json` | Create | Fixture mirroring Engram obs #2789 (`sdd/skills-validate-ondisk-gate/actuals`, revision 3), byte-identical to the live record's embedded JSON block, guarding spec.md scenario 14 with a committed artifact instead of a claim about live-only data |
