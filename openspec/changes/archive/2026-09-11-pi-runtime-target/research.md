...
- C7 `--no-extensions` disables extension discovery and `--no-skills` disables skill discovery; there is no dedicated detection API. Correction 2026-09-11 from a live `pi --help` on 0.85.1: the short aliases `-ne` and `-ns` DO exist (the earlier claim that `-ns` was absent was wrong). [S9; corrects S4]
