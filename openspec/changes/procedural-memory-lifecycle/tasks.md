...
- [ ] 0.1 Verify `main` at `engine/skills/match.go` exports `MatchCandidate` (item 30, phases 2-4)
- [ ] 0.2 Verify `openspec/changes/archive/2026-09-18-procedural-candidate-detection/tasks.md` (read-only) has every task in phases 2-4 (2.x, 3.x, 4.x) checked `[x]` (item 30 was archived 2026-09-18, PR #351)
- [ ] 0.3 Verify `skills/_shared/procedural-candidate-detection.md` on `main` contains sections 4, 5 and 6 (emission decision table, `Status` latch, `MatchCandidate`/rejection fields)
- [ ] 0.4 If any of 0.1-0.3 fails, STOP: report `status: blocked`, name the missing item, and do not start slice 1a
- [ ] 0.5 Verify `engine/skills/zero_fetch_test.go`'s `allowedImports` already includes `path` (item 30's own widening for `MatchCandidate`'s `path.Base`); slice 3a's widening (§3a.3) is additive on top of this, not a replacement
...
- [ ] 2.4 Verify: `cd engine && go test ./skills/... ./cmd/...`
...
- [ ] 3b.6 REFACTOR: confirm the rollback path removes renamed new files, leftover temps, and only-if-empty created directories (deepest first) on every injected failure point, and prints `error: rollback incomplete: <rel-path>` + exit 1 on a rollback failure that cannot be simulated cleanly (documented, not necessarily test-triggerable)
