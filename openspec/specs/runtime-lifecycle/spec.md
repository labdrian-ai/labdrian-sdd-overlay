...
Traces to: pi-runtime-target R-002, R-003, R-010
...
`pi install <local-path>`, registering it in `~/.pi/agent/settings.json`
packages. Custom agents MUST ship only under the package's `agents/`, never
written into `~/.pi/agent/agents/`.
...
Traces to: pi-runtime-target R-004, R-007
...
Traces to: pi-runtime-target R-006

`status --target pi` MUST report `partial` rather than `supported` when
package presence, in-sync build state, or longterm-mem MCP/settings.json
listing cannot be proven for any owned entry, naming the unproven entry.

#### Scenario: Unproven entry forces partial

- GIVEN any owned Pi entry (package listing, build sync, MCP visibility) cannot be proven
...
Traces to: pi-runtime-target R-007

`status --target pi` output MUST state that `pi --no-extensions` bypasses
the `before_agent_start` gate extension for that session, and that
...
- AND the `labdrian-pi` entry disappears from `~/.pi/agent/settings.json`
  ...
  removed, and gentle-pi/pi-engram-owned entries remain byte-identical

### Requirement: Pi Drift Detection via Sync-Check

Traces to: pi-runtime-target R-003, R-010

`sync-check --target pi` MUST report drift between the installed package's
built content and the current overlay manifest, using the same idiom as the
GADU generator's drift check.
...
- WHEN `sync-check --target pi` runs
- THEN it reports drift, naming the drifted entries
