...
- **GADU as a real Pi subagent.** `apply --target pi`, right after `pi install`, ensures the third-party [Pi Subagents extension](https://www.npmjs.com/package/pi-subagents-j0k3r) is present — installing `npm:pi-subagents-j0k3r` via a fixed `pi install` argv and disclosing that external dependency in its output, unless `pi-subagents` (the alternate package name) is already installed, or `LABDRIAN_PI_SKIP_SUBAGENTS=1` opts out. It then links the package's own generated `agents/GADU.md` at `~/.pi/agent/agents/GADU.md`, so GADU is dispatchable through the extension's `subagent_*` tools instead of only being a relayed persona. That link is overlay-owned: ownership is proven by `readlink` equality with the stable package path (only the inode changes across a `swap` rebuild), a pre-existing foreign file or symlink is left untouched and reported as a conflict, and `status --target pi` reports the extension's install state and the link's state (missing/current/stale/conflict) as two separate, honest entries. `runtime uninstall --target pi` removes only that symlink — never the extension package, and never any gentle-pi- or pi-engram-owned file.
