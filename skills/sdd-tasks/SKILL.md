...
- **Chain-strategy field hygiene (deprecated tokens)**: keep the `Chain strategy:` line byte-identical to the template — downstream guards match it literally — but the value you emit SHOULD be a real topology, `stacked-to-main` or `feature-branch-chain`. `size-exception` and `pending` are deprecated in this field: `size-exception` is a delivery fact (`delivery strategy: exception-ok`, no chaining, plus a recorded `size:exception`) and `pending` is a null state, not a topology. Neither is accepted by `entry-contract.schema.json`, so neither can reach an entry contract. See "Delivery and Chain Vocabulary Map" in `skills/_shared/sdd-orchestrator-workflow.md`.
