...
description: "Use this skill when starting any unit of work — a new project, a feature/chore on an existing project, or a small fix — to run the pre-SDD pipeline and hand a validated contract to the SDD engine. Triggers on: start project, new feature, implement requirement, small fix, kick off SDD, bootstrap, build this, add capability, change request, chore, hotfix, empezar proyecto, nueva feature, arreglo chico. Selects the entry tier, sequences the pre-SDD producers as delegated sub-agents, assembles and validates the entry contract, confirms readiness via the native dispatcher, hands off to the SDD engine, and fires closure-feedback after archive. Never executes producer work itself and never routes SDD phases."
license: Apache-2.0
metadata:
