...
- Source delta of commit `8c98646` against its parent `574ef92` (`git show --numstat 8c98646`, default diff algorithm): `goal.go` +21/-5, `goal_test.go` +136/-1, `testdata/v2.json` +19. `goal.go` was later rewritten by `2a2b929` (`git show --numstat 2a2b929`: +14/-112), so the cumulative diff `git diff --numstat 574ef92 2a2b929 -- engine/goal` is `goal.go` +27/-109, `goal_test.go` +136/-1, `testdata/v2.json` +19; later commit `d03f83b` (`git show --numstat d03f83b`: `goal.go` +17/-3, `goal_test.go` +62/-0) also changes `engine/goal`. Source work-unit commit `8c98646918df5688c6e67ec8546b01fa72a228b8` (`feat(goal): add versioned goal identity`) includes exactly the three reviewed Goal paths; post-commit tree blob hashes matched pre-commit bytes. Post-commit focused test spot-check passed. No remote action or Shaper implementation. Engine-only search found no non-test consumer of `Goal` struct literals; external unkeyed composite literal consumers remain unverified and could be source-incompatible with a new exported field.
